...
function highresponsive_entry_meta() {
	...
	if ( current_theme_supports( 'post-formats', $format ) ) {
		printf( '<span class="entry-format">%1$s<a href="%2$s">%3$s</a></span>',
			sprintf( '<span class="screen-reader-text">%s </span>', _x( 'Format', 'Used before post format.', 'highresponsive' ) ),
	...
	if ( ! is_singular() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( sprintf( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'highresponsive' ), get_the_title() ) );
		echo '</span>';
	}
}
endif;
...
function highresponsive_entry_date() {
	...
	printf( '<span class="posted-on"><span class="date-label">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
		_x( 'Posted on', 'Used before publish date.', 'highresponsive' ),
...
function highresponsive_entry_taxonomies() {
	$categories_list = get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma.', 'highresponsive' ) );
	if ( $categories_list && highresponsive_categorized_blog() ) {
		printf( '<span class="cat-links"><span class="screen-reader-text">%1$s </span>%2$s</span>',
			_x( 'Categories: ', 'Used before category names.', 'highresponsive' ),
			$categories_list
		);
	}

	$tags_list = get_the_tag_list( '', _x( ', ', 'Used between list items, there is a space after the comma.', 'highresponsive' ) );
	if ( $tags_list ) {
		printf( '<span class="tags-links"><span class="screen-reader-text">%1$s </span>%2$s</span>',
			_x( 'Tags: ', 'Used before tag names.', 'highresponsive' ),
			$tags_list
		);
	}
}
...
function highresponsive_entry_category_date() {
	...
	$portfolio_categories_list = get_the_term_list( get_the_ID(), 'jetpack-portfolio-type', '<span class="portfolio-entry-meta entry-meta">', esc_html_x( ', ', 'Used between list items, there is a space after the comma.', 'highresponsive' ), '</span>' );
	if ( 'jetpack-portfolio' === get_post_type() ) {
		$meta .= sprintf( '<span class="cat-links">%1$s%2$s</span>',
			sprintf( _x( '<span class="screen-reader-text">Categories: </span>', 'Used before category names.', 'highresponsive' ) ),
			$portfolio_categories_list
		);

		$meta .= '<span class="sep">' . _x( ' &ndash; ', 'Post meta separator', 'highresponsive' ) . '</span>';
	}
	...
	$meta .= sprintf( '<span class="posted-on">%1$s<a href="%2$s" rel="bookmark">%3$s</a></span>',
		sprintf( __( '<span class="date-label">Posted on </span>', 'highresponsive' ) ),
		esc_url( get_permalink() ),
		$time_string
	);
...
function highresponsive_entry_header() {
	...
	$byline = sprintf(
		...
		__( '<span class="author-label">By </span>%s', 'highresponsive' ),
	...
	echo '<div class="entry-meta">';
		...
		echo '<span class="sep">' . _x( ' &ndash; ', 'Post meta separator', 'highresponsive' ) . '</span>';
...
function highresponsive_entry_footer() {
	...
	$separate_meta = __( ', ', 'highresponsive' );
				...
				if ( 'post' === get_post_type() && 'show-meta' === $show_meta ) {
					if ( ( $categories_list && highresponsive_categorized_blog() ) || $tags_list ) {
						echo '<span class="cat-tags-links">';
							...
							if ( $categories_list && highresponsive_categorized_blog() ) {
								echo '<span class="cat-links">' . highresponsive_get_svg( array( 'icon' => 'folder-open' ) ) . '<span class="screen-reader-text">' . __( 'Categories', 'highresponsive' ) . '</span>' . $categories_list . '</span>';
							}

							if ( $tags_list ) {
								echo '<span class="tags-links">' . highresponsive_get_svg( array( 'icon' => 'hashtag' ) ) . '<span class="screen-reader-text">' . __( 'Tags', 'highresponsive' ) . '</span>' . $tags_list . '</span>';
							}
...
function highresponsive_edit_link() {
	edit_post_link(
		sprintf(
			...
			__( 'Edit<span class="screen-reader-text"> "%s"</span>', 'highresponsive' ),
			get_the_title()
		),
...
if ( ! function_exists( 'highresponsive_excerpt_more' ) ) :
	...
	function highresponsive_excerpt_more( $more ) {
		...
		$more_tag_text = get_theme_mod( 'highresponsive_excerpt_more_text',  esc_html__( 'Continue reading &gt;', 'highresponsive' ) );
...
function highresponsive_footer_content() {
	...
	<div class="site-info two">
		<div class="wrapper">
			<div id="footer-left-content" class="copyright">' .  sprintf( _x( 'Copyright &copy; %1$s %2$s. All Rights Reserved', '1: Year, 2: Site Title with home URL', 'highresponsive' ), esc_attr( date_i18n( __( 'Y', 'highresponsive' ) ) ), '<a href="'. esc_url( home_url( '/' ) ) .'">'. esc_attr( get_bloginfo( 'name', 'display' ) ) . '</a>' ) . '</div>

			<div id="footer-right-content" class="powered">' . $theme_data->get( 'Name' ) . '&nbsp;' . esc_html__( 'by', 'highresponsive' ) . '&nbsp;<a target="_blank" href="' . $theme_data->get( 'AuthorURI' ) . '">' . esc_html( $theme_data->get( 'Author' ) ) . '</a></div>
		</div><!-- .wrapper -->
	</div><!-- .site-info -->';
}
